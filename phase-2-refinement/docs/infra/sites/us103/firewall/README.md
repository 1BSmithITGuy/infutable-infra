...
The firewall runs FRR to peer with Kubernetes clusters. Each cluster advertises its service VIPs via BGP, allowing external access to LoadBalancer services.
...
- **Prefix filtering**: Each cluster can only advertise its designated VIP ranges (Route-maps with prefix-lists)
- **BFD**: Bidirectional Forwarding Detection enables sub-second failover when a BGP peer goes down.
- **Separate ASNs**: Each cluster has its own ASN for clear traffic attribution and independent routing policies.
